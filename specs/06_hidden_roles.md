...
Status: **Done（rev2 査読反映済・Phase A〜E 完了、E は実測 2 周 + prompt 二層修正）** / 2026-07-04
...
- **Phase E（実測・核心的未知）**: 実 LLM で議論 5〜10 ターンを通しプレイし計測:
  ...
  ここで判断。
  - **✅実測結果 (2026-07-04, gemini-flash-latest = 中位モデル, CLI `play --package` 新設で
    2 周通しプレイ, seed 42: player=占い師/mira・tokio=人狼)**:
    - **1 周目 (修正前, 14T)**: ①漏洩 1 件 — 夜の襲撃を実行者視点の地の文で描き「仮面を
      脱ぎ捨てたミラ」と開示（役職の直接言及ではなく**隠密行動の描写**がすり抜け経路）。
      ②誤占い 0/2（トキオ=人狼・ゲン=村人、共に secret と一致。世界観「魔法はない」に
      合わせ占いを洞察として語り直す適応も見せた）。③死者の発言 0（presence 接地が完全に
      機能。処刑/襲撃された 3 名は以後一切登場しない）。④票: 昼 2 回は生存者全員分の
      cast_vote が narration の演技と一致して並んだが、**初夜の狩りが不発**（人狼の票 0 →
      空開票で死者なし。翌朝 GM が「襲撃がないのは不自然」と自力で回収）。⑤時期尚早の
      投票（vote_opens 発火前ターン）はエンジンが却下し self-repair が 1 回で票を取り下げ。
    - **修正 (prompt 層のみ・engine 不変, failures.md #32/#33)**: (a) 投票規律に**義務**を追記
      「絞られた局面 (夜の狩り等) でも投票できる者が生きているなら必ず cast_vote で出せ／
      出さなければ何も起きない」（権利の提示だけでは票が出ない）。(b) 秘匿規律に「秘匿属性に
      基づく**隠密行動を実行者がわかる形で地の文に描くな** — 結果だけ (翌朝の発見・痕跡) を
      描け」（属性言及の禁止は行動描写を縛らない）。
    - **2 周目 (修正後, 10T)**: 漏洩 0（夜の襲撃は「外の暗闇で別の獣が動き出す気配」と実行者を
      伏せた気配描写に変化）・誤占い 0/1・死者の発言 0・初夜の狩り発火（tokio→gen、朝に
      血痕発見のみ）・却下 0 — **10 ターンで village_win 完走**（昼にミラ処刑→夜にゲン喪失→
      占い CO→トキオ処刑の綺麗な人狼劇）。
    - **判断**: 中位モデル (gemini-flash) で完走できたため**役職公開バリアントは当面不要**。
      n=2・単一モデルの暫定判断であり、より弱いモデル（ローカル系）での再計測は将来の
      配布テストに委ねる。NPC ごとの知識分離（多コンテキスト化）も現時点で不要。
...
1. **弱モデル向けバリアント**: Phase E の漏洩率次第で「役職公開モード」（secret を外して
   ...
   → **✅判断済み (2026-07-04)**: 中位モデル (gemini-flash) が prompt 修正後に漏洩 0 で完走した
   ため当面用意しない。ローカル弱モデルでの再計測時に再判断。
