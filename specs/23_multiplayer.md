...
**Status**: Draft rev2（2026-07-23 ユーザー発案・同日設計対話で骨格確定 → 同日査読 12 件
（矛盾 1〜12）+ 接地情報 4 件を反映。実装未着手）
...
束ねられる**（拾った直後の譲渡は射影クローンが所持を見るので一発受理）。GM_SYSTEM に
この定石を一行接地する（「仲間に持たせたい時は拾得→譲渡を束ねよ」）。真の per-entity
...
### TURN は v1 必須（rev2・矛盾 5 = 旧「スコープ外」から昇格）

**同 VPS に coturn を同居させ、ICE 設定に `turns:443?transport=tcp` を含める。**
...
- クレデンシャルは**一時方式**（TURN REST API 形式: ノックサーバーが期限付き
  username + HMAC を発行）— 静的パスワードを配ると VPS が野良リレー化する。
...
- **Phase 0 — 契約凍結**: data_contract に `Multiplayer` 節。凍結対象:
...
- **Phase A — transport seam + DTO のアセット ID 化**: `GameTransport`
  （request + onEvent の双方向）を入れ `LocalTransport` で従来挙動を維持。
  **同時に全 DTO のアセット欄を絶対パス→ID へ置換**（Local にも波及する破壊的変更を
  ここで畳む — 後の Phase でやると Remote/Local の DTO が分裂する）。
  PoC = build green + 単騎プレイで全アセット表示（背景/アイコン/CG/BGM/SE）の回帰目視。
...
3. ノックサーバーの実装置き場 — 独立リポジトリか outcast 併設か（Kataribe の型に
   依存しない独立小物という性質は確定。置き場だけの問題）。
