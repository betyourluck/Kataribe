...
- **Phase A（Kataribe 単独・先行可）**: install 時の `SourceMeta` 書き込み
  ...
  **起動時/一覧読込時の残骸掃除と `.bak` 自動復旧（A-3 — 掃除は update より先に入れて
  ...
  bak 破棄）。
- **Phase B（outcast・別リポ）**: `PackageSummary.sha256` 露出（一覧+詳細）+ 配信契約の
  明文化（Spec 23 追補）。Kataribe 側は `RemotePackage.sha256: Option` を先行追加
  （None 耐性 = B 未デプロイでも壊れない）。
- **Phase C（Kataribe・結線）**: `check_package_updates`（siteUrl 一致制約・失敗の沈黙）+
  ローカルタブのバッジ + `update_site_package`（プレイ中ガード / 編集検知と不一致数表示 /
  事前掃除 / スワップ復旧 / メタ更新は成功後のみ / 排他 / トースト）。
  PoC: スワップ原子性（rename 失敗の注入で旧フォルダ生存）/ 編集検知の Red→Green /
  プレイ中ガード / フォルダ名維持（zip トップ名が違っても同じパスへ・二重構造にならない）/
  失敗時に SourceMeta が旧値のまま / siteUrl 不一致メタは照会に出ない。
- **Phase D（e2e 実測）**: dev 書庫で 納本 → 取得 → 差し替え → バッジ点灯 → 上書き更新 →
  **セーブが「続きから」で生き残る**ことを通しで確認。Windows スワップの実挙動
  （スキャナ起因の rename 失敗頻度 → リトライ 1 回の要否）もここで測る。
...
2. **ローカル編集検知時 = 「上書き / キャンセル」の 2 択** + ダイアログに
   **不一致ファイル数**を表示（査読 C 採用）。「編集版を `_2` に並置」の第 3 択は
   セーブ分岐を生むので将来需要が出てから。
