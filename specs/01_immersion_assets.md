...
### #3 イベント CG → 案A デフォルト + overlay 予約
- 既定は**背景上書き・持続**（シーン切替）: `currentBackground = lastTrigger.image ?? currentLocation.image`。
...
- **背景**: 会話ペイン背景に `background`（CSS、location 変化/CG でフェード差し替え）。
