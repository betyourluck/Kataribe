...
Status: **Phase A Done + B〜D 未着手 (2026-07-15)。** Phase A = encode 分割 + fingerprint の純
リファクタ (回帰ゼロ・live 不要)、llm_client 40→42 PoC green・clippy clean。
...
- **Phase A — encode の分割 + fingerprint (純粋、live 不要) ✅ Done (2026-07-15)**:
  ...
  green 維持が回帰証明。
- **Phase B — cachedContents create/参照 + lifecycle**: create API (`POST /cachedContents`)、
  `CacheHandle` の Mutex 保持、key 照合による reuse/再作成、失効時の透過リトライ。サイズゲート +
  fallback。PoC: fingerprint 一致で create を呼ばない / 不一致で再作成 / 作成失敗で full fallback
  (fake HTTP で分岐を固定、live 不要)。
- **Phase C — config + 台帳**: `LLM_GEMINI_CACHE` / TTL / min_tokens を `LlmConfig` へ。
  data_contract / CLAUDE.md / `.env.example` / failures (#54 に「解: 本 spec」追記)。
