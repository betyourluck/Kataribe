...
Status: **Done (Phase A〜D、2026-07-15)。** Phase A = encode 分割 + fingerprint の純リファクタ
(回帰ゼロ)、Phase B = cachedContents create/参照 + lifecycle (client に wire)、Phase C = 台帳追従、
**Phase D = 実キー live 検証で目標達成** — sun_girl_ntr (総プロンプト 8000 超) で **全ターン
cachedContentTokenCount=7173** (暗黙のみだった Phase B では turn2 以降 0 = 崖)。エイリアス
`gemini-flash-latest` は cachedContents create に受理 (未決 #3 解消・具体版解決不要)、明示は
静的プレフィックス全体 (7173) をキャッシュ (暗黙の GM_SYSTEM 部分 4073 より多い)。
**Phase D で D1 のバグを 1 発で捕捉・修正**: Gemini は cachedContent 参照時に generateContent 側の
system_instruction / tools / **tool_config** のいずれも 400 で拒否 (failures #55) → tool_config も
cache 側へ移した。llm_client 40→44 PoC green・clippy clean・app backend green。
...
- **cachedContent 作成時**の body = `{model, systemInstruction, tools, toolConfig}` (静的プレフィックス)。
- **generateContent 送信時**の body = `{contents, cachedContent: <name>, generationConfig}`
  — systemInstruction / tools / **toolConfig** は**送らない** (すべて cache が持つ)。
  **⚠ 訂正 (Phase D live)**: 当初は「`toolConfig`(mode ANY) は per-request の強制指定ゆえ request
  側に残す」としていたが、**Gemini は cachedContent 参照時に system_instruction / tools /
  tool_config のいずれかが request にあると 400** (`CachedContent can not be used with
  GenerateContent request setting system_instruction, tools or tool_config`、failures #55)。
  強制指定 (mode ANY) も cache に載せる — Kataribe は常に emit_delta を強制するので cache 単位で
  一定 = 問題なし。
...
- **Phase D — live 検証 (実キー) ✅ Done (2026-07-15)**: sun_girl_ntr を gemini-flash-latest で回し、
  **総プロンプト 8000 超の全ターンで `cachedContentTokenCount=7173`** を確認 (暗黙のみだった Phase B
  では turn2 以降 0 = 崖)。→ **spec 13 の目標達成 (崖を迂回)**。結果:
  (a) **エイリアス `gemini-flash-latest` は cachedContents create に受理された** (未決 #3 解消 —
  具体版への解決は不要) (b) 静的プレフィックス ~9855 文字 (~7173 tokens) が作成でき、サイズゲート
  4000 を通過 (最小トークン閾値は下回っていない) (c/d/e = storage コスト実測 / campaign 再作成 /
  self-repair 中の cache は未計測だが機構上は fingerprint で担保) (f) **明示は 7173 をキャッシュ =
  暗黙の 4073 (GM_SYSTEM のみ) より多い** (静的プレフィックス全体が cache 対象)。
  **Phase D で D1 のバグを捕捉**: tool_config を request 側に残す設計が Gemini の 400 で判明
  (failures #55) → cache 側へ移して解消。live でしか出ない罠を初回で捕まえた。
