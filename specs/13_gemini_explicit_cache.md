...
Status: **Phase A+B Done + C〜D 未着手 (2026-07-15)。** Phase A = encode 分割 + fingerprint の純
リファクタ (回帰ゼロ)、Phase B = cachedContents create/参照 + lifecycle (client に wire、fake なし
純ロジックを PoC)。llm_client 40→44 PoC green・clippy clean・app backend check green。残 = Phase C
(台帳追従: data_contract / CLAUDE.md / .env.example) + Phase D (実キー live 検証)。
