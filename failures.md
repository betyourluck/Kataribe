...
却下されていない。②の典型は「モデルが散文で断る」(成人向け content で頻出) / CoT 混入 (#30) /
max_tokens 切断 / ops の型崩れ (#52)。なお `PROHIBITED_CONTENT` 等の**ブロックは別物**
...
【観察 (2026-07-21, ユーザー実プレイ。#63 の診断装置が即座に効いた)】やり直し理由に
...
外から見ると「GM が約束事を書かない」に見える (#63 以前はエラーが空欄で判別不能だった)。
...
(schema で配列と書いても実際には省かれる)。加えて **「LLM が X をしない」という観察は、
