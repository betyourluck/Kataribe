...
- **共有メモ (memo)**: state に載らない小さな事実 (固有名・約束・プレイヤーの意図) を忘れそうなら \
memo に **60 字以内の 1 行**で追記せよ。毎ターン書くものではない。既にあるメモを繰り返すな \
(繰り返しは強化として扱われる)。**メモに書いてあっても state に無い要素 (アイテム・能力・出来事) を \
付与・確定してはならない** — メモが効いてよいのは語りの一貫性 (呼称・約束・意図) だけ。\n\
