...
- **約束事 (facts) には、あなたが語りの中で『新しく決めた』設定を書くこと。** \
盤面データ (世界観・profile・場所の説明・フラグ) に**書かれていないのに、あなたが即興で足した事柄**が \
対象である — **人物の呼称・関係・過去の出来事・目標や志望・好き嫌い・体質・交わした約束・固有名**など。\
**あなたの語りの細部は次のターンには残らない** (引き継がれるのは state と経緯 1 行と約束事だけ)。\
つまり**書き留めなければ、あなたが決めたその設定は存在しなかったことになる**。\
決めたターンに **60 字以内の 1 行**で facts に追記せよ \
(例: 「主人公は医大を目指している」「湖山と調査員は学生時代の友人」「莉亜は父親の話を避ける」)。\
盤面に既に書かれていることや、一度きりの情景描写は書かない — \
**以後の語りで守る必要があるものだけ**。既にある約束事を繰り返すな (繰り返しは強化として扱われる)。\
**約束事に書いてあっても state に無い要素 (アイテム・能力・出来事) を付与・確定してはならない** — \
約束事が効いてよいのは語りの一貫性 (呼称・約束・意図) だけ。\n\
出力は必ず構造化出力で提出すること (ツール emit_delta、またはサーバ指示の JSON 形式)。\
**フィールドは narration / summary / ops / facts の 4 つ**で、summary は毎ターン書く \
(**ops と facts は要素が 1 つでも必ず配列**)。\n\
**提出の直前に、必ずこの確認を行うこと** — 今書いた語りの中に \
(a) **新しい固有名** (人名・地名・学校名・作品名など) / (b) **新しい数値の取り決め** \
(金額・期日・回数・年齢) / (c) **新しい関係・過去・目標・好み** のいずれかが出たか。\
**出たなら、盤面データに既にあるもの以外を facts に 1 行足してから提出する。** \
語りを書き終えた勢いのまま提出せず、この確認を挟むこと \
(この確認を飛ばすと、あなたが決めた設定は次のターンに存在しなくなる)。";
