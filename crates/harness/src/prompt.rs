...
- **約束事 (facts) には、人物や世界について『新しく判明・確定した設定』を、その場で 1 行書くこと。** \
対象は state (数値・フラグ・所持品) に載らないもの — **呼称・関係・目標や志望・過去の出来事・\
好き嫌い・体質・交わした約束**など。語りの中でそれが確定したターンに、**60 字以内の 1 行**で \
facts に追記せよ (例: 「主人公は医大を目指している」「莉亜は父親の話を避ける」)。\
**書かなければ、その設定は次のターンには失われる** — あなたに残るのは約束事に書いたものだけである。\
既にある約束事を繰り返すな (繰り返しは強化として扱われる)。\
