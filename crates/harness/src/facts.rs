...
pub fn facts_note(list: &[FactEntry]) -> Option<String> {
    if list.is_empty() {
        ...
        // **状態報告でなく指示にする** — 「まだ何もない」だけだと GM は「まだ該当が無い」の
        // 証拠と読み、毎ターン同じ判断を繰り返す自己強化ループに入る (実測 0/45・0/20 の
        // 絶対ゼロ。GM 自身がメタ質問でこのループを申告した)。
        return Some(
            "\n\n# 約束事 (プレイヤーと GM が共有している取り決め)\n\
             まだ 1 件もない。**ここまでの語りであなたが決めた設定** (人物の呼称・関係・過去・\
             目標・好き嫌い・交わした約束など、盤面データに無いのにあなたが足したもの) を \
             思い返し、以後も守るべきものがあれば、このターンで facts に拾って書くこと。\n"
                .to_string(),
        );
    }
...
mod tests {
    ...
    fn facts_note_grounds_subordination_and_sorts_by_score() {
        ...
        assert!(empty.contains("まだ 1 件もない"), "空の合図: {empty}");
        // **状態報告でなく指示**にする (「まだ無い」だけだと「該当が無い」の証拠と読まれ、
        // 毎ターン同じ判断を繰り返す自己強化ループに入る — 実測 0/45・0/20 の絶対ゼロ)。
        assert!(empty.contains("拾って書く"), "遡って拾えと指示する: {empty}");
        assert!(empty.contains("あなたが決めた設定"), "対象は GM 自身の即興: {empty}");
