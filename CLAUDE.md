...
- ⬜ 次: 実クラウド LLM での密室脱出 **通しプレイ**（`harness` bin）。`LlmClient` を `DeltaProposer` として配線し、`.env` の実キーで「制約内で構造化出力を出し続けられるか」を測る（勝ち筋の分水嶺）。`failures.md #3`（provider schema サブセット差）がここで初検証。
