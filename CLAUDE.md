...
- 合計 **61/61 green**（gm_core 33 + harness 19 + llm_client 9）、clippy clean。
...
- ⬜ 次: (a) `LoreStore` を **embedding semantic recall** へ格上げ（同 `Memoria` trait 裏で差し替え、tag 一致→意味的近傍）。usage-over-extension: tag recall が不足と接地できてから着手。(b) **Tauri+Vue UI 殻の移植**（narration の literal `\n` 正規化 `#16`、`KATARIBE_LANG` 多言語表示）。〔✅recall→prompt 注入は実装済〕
