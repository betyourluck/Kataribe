...
- 合計 **63/63 green**（gm_core 33 + harness 21 + llm_client 9）、clippy clean。
...
- ⬜ 次: (a) **Tauri+Vue UI 殻の移植**（narration の literal `\n` 正規化 `#16`、`KATARIBE_LANG` 多言語表示）。(b) 実 LLM で memoria_bridge を通しプレイし TF-IDF recall の十分性を実測。〔✅recall semantic 化（bigram TF-IDF cosine）・recall→prompt 注入は実装済。**神経 embedding** は TF-IDF が不足と接地できてから（usage-over-extension）〕
