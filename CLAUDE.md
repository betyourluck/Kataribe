...
- ⬜ **設計方針: キャンペーン化 — 密室脱出からフィールドTRPGへ (2026-06-24 設計収束・未実装)**: 単一シナリオ（1 goal + 内部場所グラフ）を、**状態を持ち越したまま authored モジュールを繋ぐ**キャンペーンに一般化する。北極星「忘れない GM」を場所移動に適用。詳細は `data_contract.yaml` の `Campaign` 節。要点:
  ...
  - 最小着地順: `transition`（状態の器・per-flag 持続性込み） → `reached()`（分岐の出口）。PoC は「状態を持ったまま骨格 swap → HP/所持品/好感度が保たれる」を Red→Green。
