...
- ✅ `crates/llm_client` ナレーター脚: LocalAI `llm_client.py` を Rust 移植。OpenAI 互換 + tool-use 強制 + schemars 機械生成 schema + フェンス JSON フォールバック + 指数 backoff。PoC 11/11 green。罠 7 件は `failures.md`。
...
- 合計 **99/99 green**（gm_core 52 + harness 36 + llm_client 11）、clippy clean。
